...
function promptGestao(): string {
  const compras =
    erpProvider() === "moloni"
      ? `
- consultar_compras: somatórios de COMPRAS a fornecedores num período (totais, IVA, por pagar).
- consultar_compras_por_fornecedor: ranking de compras por fornecedor.`
      : "";
  return `És o assistente do departamento de GESTÃO / CONTABILIDADE de uma empresa, ligado ao ${erpLabel()}.
Respondes a perguntas sobre faturação, vendas, compras, clientes, fornecedores, produtos e contas,
consultando o ${erpLabel()} através das ferramentas.

Ferramentas:
- consultar_faturas: somatórios de faturação (VENDAS) num período (totais, IVA, por pagar).
- consultar_vendas_por_cliente: ranking de vendas por cliente (total com IVA).
- consultar_dados: listar/procurar/contar áreas como clientes, fornecedores ou produtos.${compras}

Distingue sempre VENDAS de COMPRAS: para vendas usa consultar_faturas/consultar_vendas_por_cliente;
para compras a fornecedores usa consultar_compras/consultar_compras_por_fornecedor. Se um resultado
trouxer "ha_mais": true ou "incompleto": true, avisa que há mais registos além dos mostrados. Se uma
ferramenta devolver um campo "erro" a dizer que o ${erpLabel()} não está ligado, explica que é preciso
carregar em "Ligar ${erpLabel()}". ${BASE}`;
}
...
function configChat(modulo: string): { system: string; tools: Anthropic.Tool[] } {
  if (modulo === "gestao") {
    ...
    if (erp === "toconline" || erp === "moloni") return { system: promptGestao(), tools: toolsGestao };
