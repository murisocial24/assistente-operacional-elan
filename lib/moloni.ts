...
// =====================================================================
// COMPRAS (documentos de fornecedor)
//  - Faturas de fornecedor (FF): supplierInvoices/getAll  -> somam
//  - Notas de crédito de fornecedor (NCF): supplierCreditNotes/getAll -> subtraem
//  Valores COM IVA (net_value). Máximo qty=50 nestes endpoints.
// =====================================================================

async function percorrerFornecedor(
  endpoint: string,
  cb: (d: MoloniDoc) => void,
  opts: { desde?: string; maxDocs?: number } = {}
): Promise<boolean> {
  ...
  let incompleto = false;
  while (true) {
    const lote = (await apiPost(endpoint, { company_id: companyId, offset, qty })) as
      | MoloniDoc[]
      | Record<string, unknown>;
    if (!Array.isArray(lote)) throw new Error("ERRO_API");
    if (lote.length === 0) break;
    ...
    for (const d of lote) {
      cb(d);
      if (!desde || dia10(d.date) >= desde) algum = true;
    }
    offset += lote.length;
    if (desde && !algum) break;
    if (offset >= maxDocs) {
      incompleto = true;
      break;
    }
...
export async function consultarCompras(dataInicio: string, dataFim: string): Promise<ResumoCompras> {
  if (!isConnected()) throw new Error("NAO_LIGADO");
  let total = 0,
    semIva = 0,
    iva = 0,
  ...
  const acumular = (d: MoloniDoc, sinal: number) => {
    const dia = dia10(d.date);
    if (dia < dataInicio || dia > dataFim) return;
    const net = (d.net_value ?? 0) * sinal;
    const taxes = (d.taxes_value ?? 0) * sinal;
    total += net;
    semIva += net - taxes;
    iva += taxes;
    porPagar += ((d.net_value ?? 0) - (d.reconciled_value ?? 0)) * sinal;
    if (sinal > 0) n += 1;
  };

  await percorrerFornecedor("supplierInvoices/getAll", (d) => acumular(d, 1), { desde: dataInicio });
  await percorrerFornecedor("supplierCreditNotes/getAll", (d) => acumular(d, -1), { desde: dataInicio });

  return {
    data_inicio: dataInicio,
    data_fim: dataFim,
    total_comprado: arred(total),
    total_sem_iva: arred(semIva),
    total_iva: arred(iva),
    valor_por_pagar: arred(porPagar),
    numero_de_faturas: n,
...
export type ComprasPorFornecedor = {
  periodo: string;
  total_geral: number;
  numero_de_fornecedores: number;
  base: string;
  ranking: Array<{ fornecedor: string; total: number; documentos: number }>;
};

export async function consultarComprasPorFornecedor(
  dataInicio?: string,
  dataFim?: string,
  topN = 25
): Promise<ComprasPorFornecedor> {
  if (!isConnected()) throw new Error("NAO_LIGADO");
  const mapa = new Map<string, { total: number; docs: number }>();
  let totalGeral = 0;

  const acumular = (d: MoloniDoc, sinal: number) => {
    const dia = dia10(d.date);
    if (dataInicio && dia < dataInicio) return;
    if (dataFim && dia > dataFim) return;
    const nome = (d.entity_name || "(sem fornecedor)").trim();
    const valor = (d.net_value ?? 0) * sinal;
    const a = mapa.get(nome) ?? { total: 0, docs: 0 };
    a.total += valor;
    if (sinal > 0) a.docs += 1;
    mapa.set(nome, a);
    totalGeral += valor;
  ...
  const ranking = Array.from(mapa.entries())
    .map(([fornecedor, v]) => ({ fornecedor, total: arred(v.total), documentos: v.docs }))
    .sort((a, b) => b.total - a.total)
    .slice(0, Math.min(Math.max(topN, 1), 100));

  return {
    periodo: dataInicio || dataFim ? `${dataInicio ?? "início"} a ${dataFim ?? "hoje"}` : "todo o histórico",
    total_geral: arred(totalGeral),
    numero_de_fornecedores: mapa.size,
    base: "Faturas de fornecedor (FF) menos notas de crédito (NCF), com IVA.",
    ranking,
  };
}

// =====================================================================
// CICLO DE CONVERSÃO DE CAIXA (DIO, DPO, CCC)
//   DIO = valor de stock ÷ custo dos produtos vendidos × 365
//   DPO = contas a pagar a fornecedores ÷ compras do ano × 365
//   CCC = DSO + DIO − DPO
// COGS vem do cache da margem; DSO vem do cache das métricas.
// =====================================================================

async function valorInventario(companyId: number): Promise<number> {
  let valor = 0;
  let offset = 0;
  const qty = 250;
  while (true) {
    ...
    for (const prod of lote) {
      const custos = (prod.suppliers ?? []).map((x) => Number(x.cost_price ?? 0)).filter((c) => c > 0);
      const custo = custoPorRegra(custos);
      const stock = Number(prod.stock ?? 0);
      if (custo > 0 && stock > 0) valor += custo * stock;
    }
    offset += lote.length;
    if (offset >= 20000) break;
  }
...
export async function calcularCiclo(ano: number): Promise<CicloCaixa> {
  if (!isConnected()) {
    return { ligado: false, ano, valor_stock: 0, compras_ano: 0, contas_a_pagar: 0, cogs: null, dso: null, dio: null, dpo: null, ccc: null };
  }
  const companyId = await getCompanyId();
  if (!companyId) throw new Error("SEM_EMPRESA");

  // 1) Valor de stock em armazém
  const valorStock = await valorInventario(companyId);

  // 2) Compras do ano e contas a pagar a fornecedores
  const desde = `${ano}-01-01`;
  const ate = `${ano}-12-31`;
  let comprasAno = 0;
  let contasPagar = 0;
  await percorrerFornecedor(
    "supplierInvoices/getAll",
    (d) => {
      const dia = dia10(d.date);
      if (dia < desde || dia > ate) return;
      comprasAno += d.net_value ?? 0;
      contasPagar += (d.net_value ?? 0) - (d.reconciled_value ?? 0);
    },
    { desde }
  );
  await percorrerFornecedor(
  ...
  // 3) COGS (cache da margem) e DSO (cache das métricas)
  if (!cacheMargem) cacheMargem = await lerMargemDisco();
  const cogs = cacheMargem && cacheMargem.ano === ano ? cacheMargem.data.custo_total : null;
  if (!cacheMemoria) cacheMemoria = await lerCacheDisco();
  const dso = cacheMemoria && cacheMemoria.ano === ano ? cacheMemoria.data.dso : null;

  // 4) DIO, DPO, CCC
  const dio = cogs && cogs > 0 ? Math.round((valorStock / cogs) * 365) : null;
  const dpo = comprasAno > 0 ? Math.round((contasPagar / comprasAno) * 365) : null;
  const ccc = dso != null && dio != null && dpo != null ? dso + dio - dpo : null;

  return {
    ligado: true,
    ano,
    valor_stock: arred(valorStock),
    compras_ano: arred(comprasAno),
    contas_a_pagar: arred(contasPagar),
    cogs: cogs != null ? arred(cogs) : null,
    dso,
    dio,
    dpo,
    ccc,
...
// cache leve (é rápido de recalcular)
let cacheCiclo: { ano: number; data: CicloCaixa; at: number } | null = null;
const TTL_CICLO_MS = 15 * 60 * 1000;

export async function cicloComCache(ano: number, forcar = false): Promise<CicloCaixa> {
  const fresco = cacheCiclo && cacheCiclo.ano === ano && Date.now() - cacheCiclo.at < TTL_CICLO_MS;
  if (fresco && !forcar) return cacheCiclo!.data;
  const data = await calcularCiclo(ano);
  cacheCiclo = { ano, data, at: Date.now() };
  return data;
}
