...
// Meta de vendas anual (para a barra de objetivo). Definida em META_VENDAS_ANO.
export function metaVendasAno(): number | null {
  const v = process.env.META_VENDAS_ANO;
  if (!v) return null;
...
function infoGestao(): ModuloInfo {
  ...
  const ligacao: Ligacao | null =
    erp === "toconline"
      ...
      ? {
          provider: "moloni",
          statusUrl: "/api/moloni/status",
          connectUrl: "/api/moloni/connect",
          label: "Moloni",
        }
      : null;
  return {
